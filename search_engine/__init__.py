from search_engine.InvertedIndex import create_index
from search_engine.PageRank import build_graph, compute_pagerank
import os
import pickle
import json
...
class SearchEngine:
    def __init__(self):
        if not os.path.exists("Data"):
            os.makedirs("Data")
        if not os.path.exists("Data/InvertedIndex.txt"):
            create_index("Data/webpages")
            build_graph("Data/webpages")
            compute_pagerank("Data/WebGraph.txt")   
        self.invertedIndex = open("Data/InvertedIndex.txt", "r")
        with open("Data/DocIDMap.txt", "rb") as f:
            self.docIDMap = pickle.load(f)
        with open("Data/Lexicon.txt", "rb") as f:
            self.lexicon = pickle.load(f)
        self.numDocuments = len(self.docIDMap)
        
        with open("Data/PageRanks.txt", "rb") as f:
            self.pageRanks = pickle.load(f)
    ...
    def search(self, query):
        ...
        results = self.cosineScores(words, dfScores, postings)
        ranks = []
        for result in results:
            ranks.append((self.pageRanks[result], result))

        #sort pages based on PageRank and return
        return list(k[1] for k in sorted(ranks, key=lambda x:x[0]))
    ...
    #retrieve the top 100 documents based on their cosine scores
    def cosineScores(self, q, dfScores, postings):
        ...
        #retrieve top 100 scores using maxheap
        topDocs = nlargest(100, scoreList, key=lambda r:r[1])
        results = []
